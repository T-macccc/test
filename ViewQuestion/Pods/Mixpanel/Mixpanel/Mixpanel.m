...
- (NSData *)JSONSerializeObject:(id)obj//转换为NSData
{
...
- (void)identify:(NSString *)distinctId//self,self.people赋distinctId,self.people.unidentifiedQueue-->self.peopleQueue
{
...
- (void)createAlias:(NSString *)alias forDistinctID:(NSString *)distinctID//用alias替代distinctId
{
...
- (void)registerSuperProperties:(NSDictionary *)properties//superProperties += properties
{
    properties = [properties copy];
    [Mixpanel assertPropertyTypes:properties];
    dispatch_async(self.serialQueue, ^{
...
- (void)registerSuperPropertiesOnce:(NSDictionary *)properties defaultValue:(id)defaultValue//添加新key
...
- (NSString *)watchModel//apple watch
{
    NSString *model = nil;
...
- (void)setCurrentRadio
{
    dispatch_async(self.serialQueue, ^(){
        ...
        properties[@"$radio"] = [self currentRadio];//self.automaticProperties[@"$radio"]
...
- (void)reachabilityChanged:(SCNetworkReachabilityFlags)flags//在self.automaticProperties中添加key：wifi
{
...
+ (UIViewController *)topPresentedViewController//获得显示的ViewController
{
...
{//发送版本设备信息，界面进行A/B Testing更新的内容替换
    dispatch_async(self.serialQueue, ^{
...
- (void)presentSurveyWithRootViewController:(MPSurvey *)survey//生成Survey并显示
{
...
- (void)showSurveyWithObject:(MPSurvey *)survey withAlert:(BOOL)showAlert//survey与alert显示
{
    if (survey) {
        dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)markSurvey:(MPSurvey *)survey shown:(BOOL)shown withAnswerCount:(NSUInteger)count//survey答案加入properties的key：show_survey
{
...
{
    if(!sender || ([sender isKindOfClass:[UIGestureRecognizer class]] && ((UIGestureRecognizer *)sender).state == UIGestureRecognizerStateBegan )) {//有sender时，接受并识别到手势,sender为Nil时触发？
...
{//web端进行调试时会被调用
    if ([self.abtestDesignerConnection isKindOfClass:[MPABTestDesignerConnection class]] && ((MPABTestDesignerConnection *)self.abtestDesignerConnection).connected) {
        ...
        NSString *designerURLString = [NSString stringWithFormat:@"%@/connect?key=%@&type=device", self.switchboardURL, self.apiToken];//wss://switchboard.mixpanel.com/connect?key=@"0d3b501ff04db3aae70d691666bd6ff3"&type=device
        ...
        void (^connectCallback)(void) = ^{
            ...
            if (strongSelf) {
                ...
                [MPSwizzler swizzleSelector:@selector(track:properties:) onClass:[Mixpanel class] withBlock:block named:@"track_properties"];//添加键值对
            }
        };
        void (^disconnectCallback)(void) = ^{
            __strong Mixpanel *strongSelf = weakSelf;
            strongSelf.flushInterval = oldInterval;
            [UIApplication sharedApplication].idleTimerDisabled = NO;//可以锁屏
            if (strongSelf) {
                for (MPVariant *variant in self.variants) {
                    [variant execute];
                }
                ...
                [MPSwizzler unswizzleSelector:@selector(track:properties:) onClass:[Mixpanel class] named:@"track_properties"];//移除键值对
            }
        };
