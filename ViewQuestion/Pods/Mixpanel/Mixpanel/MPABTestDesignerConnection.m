...
- (void)open:(BOOL)initiate maxInterval:(int)maxInterval maxRetries:(int)maxRetries//retries
{
...
- (id)sessionObjectForKey:(NSString *)key
{
    ...
    @synchronized (_session)
    {
        id object = _session[key];//id object = _session[@"session_variant"];
...
- (void)webSocket:(MPWebSocket *)webSocket didCloseWithCode:(NSInteger)code reason:(NSString *)reason wasClean:(BOOL)wasClean//和上个方法一样
{
