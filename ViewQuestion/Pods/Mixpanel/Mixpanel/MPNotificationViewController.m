...
@property (nonatomic, assign) CGFloat circlePadding;//填充
...
- (void)viewDidLoad
{
    ...
    UIPanGestureRecognizer *gesture = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(didPan:)];//拖动手势结束时image回归原处
...
#endif
{
    return UIInterfaceOrientationMaskPortrait;//竖直状态
}

- (void)beginAppearanceTransition:(BOOL)isAppearing animated:(BOOL)animated//过度动画
{
...
- (void)didPan:(UIPanGestureRecognizer *)gesture
{
    if (gesture.numberOfTouches == 1) {
        if (gesture.state == UIGestureRecognizerStateBegan) {
            ...
            CGPoint translation = [gesture translationInView:self.view];//位置获得
            self.imageView.layer.position = CGPointMake(0.3f * (translation.x) + _viewStart.x, 0.3f * (translation.y) + _viewStart.y);//self.imageView的位置拖动
        }
    }
    ...
    if (_touching && (gesture.state == UIGestureRecognizerStateEnded || gesture.state == UIGestureRecognizerStateCancelled)) {
        ...
        CGPoint viewDistance = CGPointMake(viewEnd.x - _viewStart.x, viewEnd.y - _viewStart.y);//cgPoint起点终点距离
        CGFloat distance = (CGFloat)sqrt(viewDistance.x * viewDistance.x + viewDistance.y * viewDistance.y);//根据cgpoint计算两点距离
        [UIView animateWithDuration:(distance / 500.0f) delay:0.0f options:UIViewAnimationOptionCurveEaseOut animations:^{//动画执行
            self.imageView.layer.position = self->_viewStart;//取消位置拖动
        } completion:nil];
    }
}
...
- (void)viewDidLoad
{
    ...
    self.bodyLabel.lineBreakMode = NSLineBreakByWordWrapping;//自动换行，文字自适应
    self.bodyLabel.numberOfLines = 0;

    if (!self.backgroundColor) {
        self.backgroundColor = [UIColor mp_applicationPrimaryColor];//?
        if (!self.backgroundColor) {
            self.backgroundColor = [UIColor mp_darkEffectColor];//黑背景
        }
    }

    UIColor *backgroundColorWithAlphaComponent = [self.backgroundColor colorWithAlphaComponent:0.95f];//透明度
    self.view.backgroundColor = backgroundColorWithAlphaComponent;

    if (self.notification != nil) {//有值时赋值，无值时隐藏
    ...
    [self.circleLayer setNeedsDisplay];//会调用drawRect;
...
- (void)viewWillLayoutSubviews
{
    ...
    if ([self respondsToSelector:@selector(viewWillTransitionToSize:withTransitionCoordinator:)]) {
        ...
        double angle = [self angleForInterfaceOrientation:[self interfaceOrientation]];//orientation 角度转换
...
#if __IPHONE_OS_VERSION_MIN_REQUIRED >= 80000
        topFrame = topView.frame;//keyWindow 的子视图
#elif __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000
        if ([self respondsToSelector:@selector(viewWillTransitionToSize:withTransitionCoordinator:)]) {
...
- (void)animateImage
{
    ...
    CGRect after = CGRectMake(0.0f, 0.0f, imageViewSize.width + (_circleLayer.circlePadding * 2.0f), imageViewSize.height + (_circleLayer.circlePadding * 2.0f));//padding填充

    ElasticEaseOutAnimation *circleAnimation = [[ElasticEaseOutAnimation alloc] initWithStartValue:before endValue:after andDuration:duration];//elastic弹性
...
- (void)drawInContext:(CGContextRef)ctx
{
    ...
    CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceGray();//会创建一个设备相关的灰度颜色空间的引用
    CGFloat components[] = {
        1.0f, 1.0f,
        1.0f, 1.0f,
        1.0f, 0.9f,
...
@implementation ElasticEaseOutAnimation//Elastic弹性
...
{//v,steps,increment,ceil
    NSUInteger steps = (NSUInteger)ceil(60 * self.duration) + 2;//ceil为向上取整0.2*60+2->4
	NSMutableArray *valueArray = [NSMutableArray arrayWithCapacity:steps];//4
    const double increment = 1.0 / (double)(steps - 1);//0.33
    double t = 0.0;
    ...
    NSUInteger i;
    for (i = 0; i < steps; i++) {
        float v = (float) -(pow(M_E, -8*t) * cos(12*t)) + 1; //Cosine wave with exponential decay指数衰减余弦波 pow:e的-8t次方
