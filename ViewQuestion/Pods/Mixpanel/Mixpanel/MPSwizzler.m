...
static void mp_swizzledMethod_2(id self, SEL _cmd)//self表当前对象,_cmd表当前方法
{
...
+ (void)load//NSMapTable对象初始化,其key:count,capacity,keyoptions,valueOptions,matation突变,growLock,shouldRehash
{
...
{
    return (MPSwizzle *)[swizzles objectForKey:MAPTABLE_ID(aMethod)];//获得swizzles[@"aMethod"]
}
...
{
    [swizzles setObject:swizzle forKey:MAPTABLE_ID(aMethod)];//add/replace value
}

+ (BOOL)isLocallyDefinedMethod:(Method)aMethod onClass:(Class)aClass//检查本地是否含有该方法，aclass:UIControl,
{
    uint count;
    BOOL isLocal = NO;
...
{//aMethod != nil,swizzle.blocks 添加键值对，aMethod == nil,self添加键值对
    Method aMethod = class_getInstanceMethod(aClass, aSelector);
    if (aMethod) {
        uint numArgs = method_getNumberOfArguments(aMethod);
        if (numArgs >= MIN_ARGS && numArgs <= MAX_ARGS) {//>=2&&<=4

            BOOL isLocal = [self isLocallyDefinedMethod:aMethod onClass:aClass];
            IMP swizzledMethod = (IMP)mp_swizzledMethods[numArgs - 2];//获得方法的实现
            MPSwizzle *swizzle = [self swizzleForMethod:aMethod];//swizzles[@"aMethod"];

            if (isLocal) {
                if (!swizzle) {//swizzle初始化 swizzledMethod->aMethod->originalMethod->swizzle->self,
                ...
                IMP originalMethod = swizzle ? swizzle.originalMethod : method_getImplementation(aMethod);//return a implementation of a Method
                ...
                MPSwizzle *newSwizzle = [[MPSwizzle alloc] initWithBlock:aBlock named:aName forClass:aClass selector:aSelector
                                                          originalMethod:originalMethod withNumArgs:numArgs];
...
{//aClass,aSelector->aMethod->swizzle,swizzle.originalMethod
    Method aMethod = class_getInstanceMethod(aClass, aSelector);
...
{//aClass,aSelector->aMethod->swizzle,
    Method aMethod = class_getInstanceMethod(aClass, aSelector);
    MPSwizzle *swizzle = [self swizzleForMethod:aMethod];//swizzles[@"aMethod"]
    if (swizzle) {
        if (aName) {
        ...
        if (!aName || [swizzle.blocks count] == 0) {//如果aName == nil,移除所有
            method_setImplementation(aMethod, swizzle.originalMethod);//?
            [self removeSwizzleForMethod:aMethod];
        }
    }
