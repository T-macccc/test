...
static void *MPAllocBufferForObjCType(const char *objCType)
{
    ...
    int result = posix_memalign(&buffer, MAX(sizeof(void *), alignment), size);//字节对齐
    if (result != 0) {
...
- (void)mp_setArgument:(id)argumentValue atIndex:(NSUInteger)index
{
    ...
    if ([argumentValue isKindOfClass:[NSNumber class]] && strlen(argumentType) == 1) {//arguementValue 是NSNumber classa 且长度为一
    ...
    {//NSValue
        ...
        void *buffer = MPAllocBufferForObjCType([valueArgument objCType]);//字节对齐
