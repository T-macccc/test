...
#define MPTweakSegmentName "__DATA"//段名
#define MPTweakSectionName "MPTweak"//节名
...
#define __MPTweakConcat_(X, Y) X ## Y//concat 连接数组
...
((^{ \
  /* store the tweak data in the binary at compile time. 在编译时将tweak数据以二进制储存*/ \
...
\
  /* find the registered tweak with the given name. 用名字找以注册的tweak*/ \
...
((^{ \
  /* returns a correctly typed version of the current tweak value 返回一个正确的tweak版本值*/ \
